...
└──────────────────────────────────────────────────────────────────────────┘
```
*(Figure 10: GRUT derivation chain. Generated by `uploads/make_figures.py::fig10_grut_chain()`.)*
...
══════════════════════════════════════════════════════════════════
  ...
  [1] UNIFY QM AND GR
      ...
      GR = constitutive equation, ωτ₀ → ∞ limit (linearized gravity derived,
           nonlinear ladder 4/8 rungs closed)
  ...
  [3] EXPLAIN THE DARK SECTOR
      ...
      Same medium, same equation, two frequency regimes.
  ...
  [5] MAKE NEAR-TERM FALSIFIABLE PREDICTIONS
      ─────────────────────────────────────────
      Status: SIX PREDICTIONS, TESTABLE BEFORE 2030
      ...
      F5: μ−1 = +33% on horizon scales  (DESI Y1+, 3σ now)
...
```
             ...
             │            │            │
             ▼            ▼            ▼
     Λ_grav (689 Hz)   H_inf        Ω_dm
     falsifier F1     = (2−R)/Sτ₀  = α_vac = 1/3
     confirmed/killed  → Ω_Λ = 0.69  (from n_g refractive
                       → H₀ = 69     enhancement)
                       km/s/Mpc
```
...
| Honest negatives documented | Rare | Occasional | Occasional | **34 corrections, all public** |
| Near-term predictions | Few / Planck-scale | Few | Few | **Six predictions, testable before 2030** |
...
**Three additions from v2 (original v3 scope):**
...
**Supporting additions (original v3 scope):** Two-route convergence gap formally registered in Ch 7 registry footer. Z₃/Koide sector results expanded from a table row to a dedicated subsection in Ch 12.

**v3 pre-upload additions (June 2026 Zenodo preparation):** Four strengthening additions made before Zenodo deposit to address reviewer-facing gaps in derivation clarity:
...
| **σ₈ (Poisson-only μ modification)** | **0.836–0.837 at fixed ΛCDM parameters (+3.1–3.2%)** | **0.811 ± 0.006 (Planck ΛCDM posterior)** | **Fixed-background parameter response: +3.1% shift computed (two-solver agreement: ODE +3.13% + Correction #36 +3.22%); fixed-param deviation ≈ 4.3σ from ΛCDM posterior — NOT a cosmological tension without joint parameter refit; CLASS needed for gauge confirmation** | **Anchored — parameter response computed; cosmological significance unresolved (June 2026)** | **9** |
...
| **CMB low-ℓ ISW enhancement** | **No physical prediction established** | — | **Metric-consistent CAMB v2 shows zero ISW excess; v1 prototype excess was artifact; physical prediction requires CLASS Newtonian gauge implementation** | **Not yet a GRUT prediction (June 2026)** | **9** |
...
**Routes and their status.** Three independent derivation routes for R are summarized in the table below and shown graphically in Figure 3 above.
