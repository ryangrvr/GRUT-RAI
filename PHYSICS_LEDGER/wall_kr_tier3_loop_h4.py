...
def save_stage(tag):
    ...
    json.dump(RESULT, open(os.path.join(
        HERE, "T3_05_H4_%s_STAGE_RESULT.json" % tag.upper()), "w"),
...
_HKILL = {H**n: sp.Integer(0) for n in range(7, 13)}
...
if STAGE == "assemble":
    ...
    json.dump({"ret_wigner": sp.srepr(RET), "nk_wigner": sp.srepr(NKC),
              ...
              open(os.path.join(HERE, ".tier3_h4_integrand_cache.json"), "w"))
...
if STAGE == "flat":
    ...
    IC = json.loads(open(os.path.join(
        HERE, ".tier3_h4_integrand_cache.json")).read())
...
if STAGE == "freeze":
    ...
    for tag in ("REDUCE", "ASSEMBLE", "FLAT", "GRADE"):
        p = os.path.join(HERE, "T3_05_H4_%s_STAGE_RESULT.json" % tag)
    ...
    merged["hard_stop"] = ("NO final K_R, NO G_R^TT dressing, NO D5, NO "
                           ...
                           "owner inspection.")
    outp = os.path.join(HERE, "T3_05_H4_LOOP_RESULT.json")
