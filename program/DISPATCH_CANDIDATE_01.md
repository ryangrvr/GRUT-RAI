...
> # ⛔ RETIRED AS A DISPATCH CANDIDATE — owner disposition, 2026-09-09.
>
> **SLOT formal map: VALID** under the weak governance reading.
> **SLOT source fidelity: FAIL.** **SLOT dispatchability: FAIL.**
>
> **Not to be repaired or rewritten into a new candidate** — that would create a new selection
> object and bypass the selection gate. Any source-native reformulation must enter candidate mining
> from the start, on its own merits.
>
> **The CLPW source findings in §8 are PRESERVED as reusable evidence for Q3** and are not retired
> with the candidate. What failed is the corpus's *reading* of that material, not the material.
>
> **METHODOLOGICAL RESULT, recorded as the standing lesson of this pass:** a two-way governance map
> is **necessary but not sufficient**. An external-source candidate must additionally be the
> scientific question the source actually poses. The selection machinery worked — it prevented an
> external investigation being spent on a fork the corpus had partly constructed.
>
> Q3 remains **OPEN**. Ledger unchanged. Nothing sent. Not pushed.

> # ⛔ SOURCE CHECK RUN 2026-09-09 — **FAIL ON DISPATCH FIDELITY.** The question as written is
> # NOT supported by the primary source. Not sendable. Not repaired by interpretation.
>
> arXiv:2206.10780v5 §1.3, §2.4, §4.2, §4.3 read against the draft. Findings in §8 below.
> **Two independent clauses are unsupported**, and a third finding is sharper than expected: the
> fork's answer is already visible in **§2.4**, without §4.3, and it is the *hostile* billing.
> Per the owner ruling — *"if the source does not support the proposed fork exactly, mark the
> candidate failed rather than repairing it by interpretation"* — the candidate is marked **FAILED
> FOR FIDELITY**. It is **not** marked scientifically dead: §4.3 carries a real open question, in
> CLPW's own words, which is **not** the question this dispatch asks.
>
...
## 8. SOURCE-GROUNDING PASS — arXiv:2206.10780v5 (run 2026-09-09)

**Scope held:** primary source read only. No computation, no SLOT calculation, no new primitive, no
ledger change, no dispatch sent, Q3 untouched. Nothing below repairs the question by interpretation.

### 8.1 A — WHAT CLPW PROVES

**§2.4, the observer construction.** The observer is an explicit added degree of freedom, not a
bookkeeping device:
- `H_obs = q`, with *"we will assume that the energy of the observer is non-negative, so we will
  assume that q ≥ 0. Thus the Hilbert space of the observer, in this model, is H_obs = L²(ℝ₊)."*
- **Eq (20)**: `Ĥ = H + H_obs = H + q` — the observer enters **the gravitational constraint**.
- **Eq (21)**: `Â = (𝒜 ⊗ B(L²(ℝ₊)))^Ĥ`, the crossed product of `𝒜` by the one-parameter
  automorphism group generated by `H`.
- The necessity is stated flatly: *"Since 𝒜^H is trivial, the only way to get anything sensible is
  to include the degrees of freedom of the observer as part of the analysis."*

**§1.3, the continuous dimension.** *"Murray and von Neumann showed that the Hilbert space
representations of the algebra 𝒜 are classified by a parameter d, called the continuous dimension,
that can be any positive real number or ∞."* **`d` is defined on (0, ∞]. There is no `d = 0` case in
the classification** — which is exactly why §4.3 calls `d = 0` problematic.

**§4.2.** The construction works **given** a second observer behind the horizon *"who is completely
or almost completely entangled with the observer in the static patch."* The `G_N → 0` limit is
simple **there**.

### 8.2 B — WHAT CLPW CONJECTURES (and the draft must not upgrade)

All of §4.3's quantitative content is explicitly conditional:
- *"An entropy deficit of roughly (1/2)log(1/G_N) would then correspond to a continuous dimension
  d ~ G_N^{1/2}."*
- *"**If this is the correct answer**, then d ~ G_N^{1/2}."*
- *"Having d = 0 would mean that there is no Hilbert space on which Â acts, which seems problematic
  in the G_N → 0 limit."*
- *"To determine d more precisely would require taking into account quantum corrections in G_N,
  which go beyond the scope of the present analysis."*

### 8.3 C — WHERE THE OBSERVER / CLOCK ENTERS

**The clock language is the paper's own**, not corpus shorthand:
- *"−p is the time told by the observer's clock."*
- *"The role of the observer was to help us fix the time translation symmetry of de Sitter space, so
  an observer is any system that can tell time."*

So the clock enters **explicitly, into the constraint (eq 20), as a separate Hilbert-space factor
(eq 21), for the stated purpose of fixing the time-translation symmetry.**

### 8.4 D — IS THE TWO-WAY FORK SUPPORTED? **NO.**

**The paper contains no input-counting framing at all.** It never asks whether the observer is a
*"second input"* versus a *"relative temporal datum"*. The observer is **posited as a necessary
degree of freedom** to make the constraint algebra non-trivial.

**And the sharper finding:** on the source's own construction the answer to the corpus fork is
already visible in **§2.4** — the clock **is** a supplied system gauged into the constraint, as a
separate factor. That is the corpus's *hostile* billing, and it is simply **what CLPW does**. It is
not in question in the source and does not require §4.3 to see.

**CLPW's actual §4.3 open question, in their words:** *"In the absence of such a second observer, the
question [of how Ĥ emerges as the space of physical states] is more difficult and appears not to
have a simple limit for G_N → 0."* That is a question about **how the physical-state space emerges
and what `d` is** — not about counting irreducible inputs.

### 8.5 E — IS "RELATIVE TEMPORAL DATUM ALONE" MEANINGFUL IN THE SOURCE? **NO.**

The phrase is **corpus terminology and is not grounded in CLPW**. The source's structure is: crossed
product by the modular automorphism group generated by `H`, with the observer's `q` in the
constraint. No alternative in which a *"relative temporal datum alone"* carries the construction is
presented, contemplated, or named.

### 8.6 VERDICT — **FAIL**

Three unsupported clauses in the outgoing question:

| Clause | Why it fails |
|---|---|
| *"does §4.3's single-patch G_N → 0 limit **go through rigorously**"* | **Presupposes CLPW claims a limit there.** The source says the opposite: *"appears not to have a simple limit for G_N → 0."* |
| *"...need a supplied clock gauged into the constraint **as a separate input**"* | **Not an open question in the source.** §2.4 gauges the clock into the constraint by construction and calls it the only way to get anything sensible. |
| *"or is the surviving input **the relative temporal datum alone**"* | **Corpus terminology, absent from CLPW.** No such alternative exists in the source's formalism. |

**Not repaired here.** Per the owner ruling, an unsupported fork is marked failed rather than
rewritten into support. Restating the question so that it matches §4.3's actual open content would
produce a **different object**, whose two-way map into program state would have to be established
from scratch — that is a new selection, not a wording repair, and it is the owner's call.

**What survives, and it is not nothing.** §4.3 carries a genuine, live, author-acknowledged open
question; the clock/observer genuinely enters the constraint and is genuinely described as fixing the
time-translation symmetry; and `d = 0` is genuinely outside the Murray–von Neumann classification.
The corpus's *reading* of that material is what failed, not the material.
