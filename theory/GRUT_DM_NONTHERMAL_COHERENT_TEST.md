...
**Tier: CONJECTURAL / foundational-extension.** A clean NEGATIVE is a first-class result, banked exactly like the 4.71 keV warm shear phonon.
...
- F(t) is BENEATH the vacuum. Per `GRUT_V4_SPINE.md:670` a foundational extension beneath the vacuum "constitutes a different theory, not GRUT." Any POSITIVE here is a **SUCCESSOR-THEORY hook** (GRUT growing a new layer downward), **NEVER upgraded to DERIVED**. F(t) sits outside the no-gos' jurisdiction *precisely because* the no-gos are theorems about the vacuum's action, not the substrate under it.
...
**Conclusion (Task C):** the candidate requires none of (i) a 2nd propagating vacuum pole, (ii) a 1/k² nonlocal inverse-Laplacian in the matter→metric kernel, (iii) a new relaxational vacuum pole. The no-gos have NO JURISDICTION over F(t) — they are theorems about the vacuum's action, and F(t) is the substrate beneath it (`SPINE.md:311-312`). This is precisely why a positive here would be a SUCCESSOR-THEORY hook, never a GRUT derivation.
...
**The win (the specification upgrade).** The single-mode/no-DM conclusion moves from "single-mode postulate + warm-relic miss" to a **specified-and-unsourced successor-theory shopping list.** The missing host-theory of F(t) must supply, all beneath the vacuum (so it trips no theorem-wall):
...
**Falsifier.** The candidate (or any successor filling the slot) is falsified if the wave-DM soliton-core scale is excluded observationally: small-scale power suppression vs **Lyman-α forest + Milky-Way dwarf-satellite counts** pushing the fuzzy-DM mass floor above ~2×10⁻²¹ eV and closing the kpc-core window. If the clustering window itself is observationally eliminated, the successor slot is empty regardless of what F(t) could supply — the impossibility becomes total rather than sharpened.

**Tier.** CONJECTURAL / foundational-extension. Even a positive here would be a SUCCESSOR-THEORY hook (GRUT growing a layer downward), never DERIVED. This is a clean NEGATIVE — a first-class result, banked exactly like the 4.71 keV warm shear phonon.

---

**Better DEFINITION of dark matter (GRUT's tiered ontology):** *Dark matter is a HOSTED, no-go-clean coherent scalar living in the pre-responsive substrate F(t) beneath the vacuum — gravitating by ordinary GR stress-energy, carrying one new intermediate mass scale (~10⁻²² eV, in the empty middle of GRUT's 34-order τ-gap) and one independent abundance amplitude — neither of which GRUT's current bath specification forces; it is specified-and-unmet, not derived.*
