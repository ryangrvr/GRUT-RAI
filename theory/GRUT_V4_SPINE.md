...
the highest-fanout claim in the registry — 32 downstream dependencies. *(`ctp_action_structure`,
...
fanning out maximally: a ±0.07 shift in α moves Ω_Λ by roughly a factor of ~2 through the
S = 12π/α² screening (`.venv` check: Ω_Λ,tree = (2−R)² runs 0.95 → 0.71 → 0.53 across
α = 0.26 → 1/3 → 0.40), so the value's standing is decisive. *(`alpha_vac_axiom`,
...
- **Baryogenesis (existence derived).** The CTP path is asymmetric (the canonical ratio
  R ≠ 1), and a nonzero baryon-to-photon ratio is forced by that asymmetry — it would vanish
  identically at R = 1. The native structural factor (2 − R_B)/S_B, multiplied by hosted SM
  inputs (Jarlskog J_CP, electroweak nonequilibrium K_neq), lands η_B ≈ 6.57 × 10⁻¹⁰, about
  +7.7% from the Planck 6.1 × 10⁻¹⁰ (`baryogenesis_eta_b`, registry.py:1696, computed). —
  **DERIVED `[SPLIT]`**: existence derived (Sakharov-CP content), magnitude assisted by hosted
  inputs. **FLAG (Appendix A #4):** the 7.7% agreement must not be read as zero-parameter.
...
Δm²_atm/Δm²_sol ≥ 194.7, against the observed 33.9 — a factor of ~6 too large, computed and
