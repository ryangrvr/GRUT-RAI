...
| 4 | **Gravity** | Partial | Semiclassical response sector, USL from CTP influence functional | Test constitutive metric couplings, toy backreaction | Backreaction / graviton / quantum gravity completion |
...
| Tag | Meaning |
|-----|---------|
| **Recovered** | Derived, computed, and verified within the GRUT framework. Matches known physics. |
| **Predictive** | Makes specific, testable, zero-parameter predictions beyond known physics. |
| **Partial** | Some structural results established; key closure conditions remain. |
...
Every sector distinguishes:

- **Result** — already derived, computed, or verified
- **Entry point** — a legitimate extension path from existing GRUT machinery
- **Open question** — interesting but not yet anchored for claim status

## Build Order

1. Gravitational decoherence (DONE — template sector)
2. Quantum mechanics recovery (largely done, needs packaging)
3. Electroweak host structure (largely done, needs packaging)
4. Gravity (partial — needs honest scoping)
5. QCD entry point (new work)
6. Cosmology (partial — needs honest scoping)
7. Flavor / neutrinos (open — entry points only)
8. Dark matter / baryogenesis (frontier — entry points only)
9. Coupling unification (open)
10. Quantum gravity (explicit open gate)

Each sector follows the gravitational decoherence template:
- Constitutive entry point
- Computational notebook / solver module
- Validation tests
- Adversarial checks
- Closure condition stated explicitly
